...
def get_att_vec(input_mat, input_vec, v_length = 64):
	shape_m = input_mat.shape.as_list()
	shape_v = input_vec.shape.as_list()
	wmr = tf.Variable(tf.truncated_normal([shape_m[2], v_length]), dtype = tf.float32)
	wvr = tf.Variable(tf.truncated_normal([shape_v[1], v_length]), dtype = tf.float32)
	part1 = tf.reshape(tf.matmul(tf.reshape(input_mat, [-1, shape_m[2]]), wmr), [-1,shape_m[1],v_length])
	part2 = tf.reshape(tf.matmul(input_vec, wvr), [-1, 1, v_length])
	v = tf.Variable(tf.truncated_normal([v_length, 1]), dtype = tf.float32)
	co = tf.reshape(tf.matmul(tf.reshape(tf.tanh(part1+part2), [-1, v_length]), v), [-1, shape_m[1], 1])
	return tf.reduce_sum(input_mat*co, axis = 1)

...
class batch_generator():
	...
	def next(self):
		if self.cur_pos + self.batch_size < self.length:
			...
			return np.array(u), np.array(q), np.array(y), pos
		else:
			record = self.cur_pos
			self.cur_pos = self.batch_size - self.length + self.cur_pos
			if self.cur_pos == 0:
				return np.array(self.train_u[record:self.length]), np.array(self.train_q[record:self.length]), np.array(self.train_y[record:self.length]), np.array(self.dialog_pos[record:self.length])

			return np.concatenate((self.train_u[record:self.length], self.train_u[:self.cur_pos])), np.concatenate((self.train_q[record:self.length], self.train_q[:self.cur_pos])), np.concatenate((self.train_y[record:self.length], self.train_y[:self.cur_pos])), np.concatenate((self.dialog_pos[record:self.length], self.dialog_pos[:self.cur_pos]))

...
from batch_generator_long_bidir import batch_generator
from keras.layers import MaxPooling2D
from glove_embedding import glove_embedding
from character_gating import character_gating
import numpy as np
import pickle
import time
import os
...
#####################
dialog_index1 = np.zeros([batch_size, dialog_length], np.int32)
dialog_index2 = np.zeros([batch_size, dialog_length], np.int32)

##尝试 1 stat
dialog_forward_raw = LSTM(64, return_sequences = True)(utter)
dialog_backward_raw = LSTM(64, return_sequences = True)(utter)
...
dialog_raw = Attention_BiGru(utter, query, return_sequences = True)
##尝试2 end
'''

shape = dialog_index1.shape
dialog_forward = [dialog_forward_raw[i,dialog_index1[i,j],:] for i in range(shape[0]) for j in range(shape[1])]
dialog_backward = [dialog_backward_raw[i,dialog_index2[i,j],:] for i in range(shape[0]) for j in range(shape[1])]
dialog_forward = tf.reshape(tf.stack(dialog_forward), [batch_size, dialog_length, -1])
dialog_backward = tf.reshape(tf.stack(dialog_backward), [batch_size, dialog_length, -1])
dialog = tf.concat([dialog_forward, dialog_backward], axis = -1) 
...
with tf.name_scope("concat_layer"):
    ...
    hidden_layer = tf.concat([hq, ad, aq, att_gru_qd.get_result()], -1, name="hidden_layer") 
...
"""
label_id = []
for l in sorted(label2id.items(), key = lambda x : x[1]):
    label_id.append(word2id[l])
label_id = np.reshape(label_id, [num_classes, 1])
C = tf.nn.embedding_lookup(embedding_matrix_converted, label_id) # num_classes * embedding_dim
C = tf.transpose(C, [1, 0])
print("shape of C:", C.shape.as_list())
"""

#prediction = character_gating(hidden_layer, gating_size, hidden_size, num_classes, batch_size)

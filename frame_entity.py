...
from batch_generator_entity import batch_generator
from keras.layers import MaxPooling2D
from glove_embedding import glove_embedding
from aoa import get_aoa_attention
from pos_attention import attention_pos
import numpy as np
import pickle
import time
import os
...
"""
batch = batch_generator(U, Q, labels, word2id, vocab_size, list(label2id.values()), batch_size, query_length, utter_length, dialog_length, word_class, label2id)
batch_dev = batch_generator(U_dev, Q_dev, labels_dev, word2id, vocab_size, list(label2id.values()), batch_size, query_length, utter_length, dialog_length, word_class, label2id)
batch_tst = batch_generator(U_tst, Q_tst, labels_tst, word2id, vocab_size, list(label2id.values()), batch_size, query_length, utter_length, dialog_length, word_class, label2id)
...
pos = np.zeros([batch_size, num_classes, 1])

#####################
#这里填充模型，得到attention向量
#####################、

d = Bidirectional(LSTM(hidden_length, return_sequences = True))(utter)
q = Bidirectional(LSTM(hidden_length, return_sequences = True))(query)
d = tf.reshape(d, [batch_size, utter_length, 2 * hidden_length])
q = tf.reshape(q, [batch_size, query_length, 2 * hidden_length])
attention = get_aoa_attention(d, q, batch_size)
res = attention_pos(pos, attention, batch_size, num_classes)


#saver = tf.train.Saver(max_to_keep = 4)

with tf.name_scope("optimizer_and_loss"):
	loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = res, labels = labels))
...
correct_prediction = tf.equal(tf.argmax(res, axis = 1),tf.argmax(labels, axis = 1))

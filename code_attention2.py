...
with tf.name_scope("dialog_attention_layer"):
	...
	dialog2 = Conv1D(length_1D, window_length, activation = "relu", name = "D2")(dialog) #D2
        Wq = tf.Variable(tf.truncated_normal([hidden_length * 2, attention_length], -1, 1))
        Wd = tf.Variable(tf.truncated_normal([hidden_length * 2, attention_length], -1, 1))
        dialog = tf.unstack(dialog)
        query = tf.unstack(query)
        dialog2 = tf.stack([tf.matmul(dialog[i], Wd) for i in range(batch_size)])
        query2 = tf.stack([tf.matmul(query[i], Wq) for i in range(batch_size)])
